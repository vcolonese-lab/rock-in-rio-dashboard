...
function exportXLS() {
  ...
  // Build lookup map from rawShows: key = "eventSuffix\x00date\x00time" -> show entry
  // Using \x00 (null char) as separator to avoid conflicts with event names that contain '|'
  ...
  for (const r of _rawShows) {
    const suffix = extractSuffix(r.eventName, r.local);
    const key = `${suffix}${SEP}${r.date}${SEP}${r.time}`;
